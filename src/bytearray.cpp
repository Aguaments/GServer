...
namespace agent{
    ...
    bool ByteArray::isLittleEndian() const{
        return m_endian == AGENT_BYTE_ORDER;
    }

    void ByteArray::setIsLittleEndian(bool val){
        if(val){
            m_endian = AGENT_LIMITE_ENDIAN;
        }else{
            m_endian = AGENT_BIG_ENDIAN;
        }
    }
    ...
    void ByteArray::writeInt32(int32_t val){
        writeUint32(EncodingZigzag32(val));
    }
    ...
    void ByteArray::read(void* buf, size_t size, size_t position) const{
        ...
        size_t npos = position % m_basesize; // 当前块处理的pos
        size_t ncap = m_cur -> size - npos;
        size_t bpos = 0; // 当前读取的进度相较于size的偏移
    ...
    bool ByteArray::writeToFile(const std::string& name) const {
        ...
        if(!ofs) {
            AGENT_LOG_ERROR(g_logger) << "writeToFile name=" << name
    ...
    bool ByteArray::readFromFile(const std::string& name) {
        ...
        if(!ifs) {
            AGENT_LOG_ERROR(g_logger) << "readFromFile name=" << name
                << " error, errno=" << errno << " errstr=" << strerror(errno);
            return false;
        }
